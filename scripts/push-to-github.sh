...
# --- per-wave files: Phase 2D Orax host discovery + pairing UI ---
git add artifacts/mustaflow/src/App.tsx 2>/dev/null || true
git add artifacts/mustaflow/src/pages/mode-select.tsx 2>/dev/null || true
git add artifacts/mustaflow/src/pages/orax-devices.tsx 2>/dev/null || true
...
git add artifacts/ora-mobile/lib/api.ts 2>/dev/null || true
git add artifacts/ora-mobile/lib/types.ts 2>/dev/null || true
