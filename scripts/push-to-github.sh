...
git add \
  ...
  artifacts/api-server/src/routes/project-activity.ts \
  artifacts/mustaflow/src/lib/builder-completion.test.ts \
  artifacts/mustaflow/src/lib/builder-completion.ts \
  artifacts/mustaflow/src/pages/projects/components/activity-log-tab.test.tsx \
  artifacts/mustaflow/src/pages/projects/components/queue-progress-strip.tsx \
  .agents/memory \
  docs/changelog.md \
