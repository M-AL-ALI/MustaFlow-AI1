...
**Symptom (seen in prod):** Ora plan-upgrade screen shows "Ora plans are not configured — STRIPE_CORE_PRICE_ID is not configured" on the LIVE site = the price-ID env vars were set only in `development` scope (test IDs); production scope was empty. Live products/prices/webhook may already exist — the only gap is the missing `production`-scoped env vars. Fix = `setEnvVars({values:{STRIPE_CORE_PRICE_ID, STRIPE_WAVE_PRICE_ID}, environment:'production'})` with the LIVE price IDs, then **republish** (running deployment bakes env at deploy time — new prod vars need a redeploy to take effect).
