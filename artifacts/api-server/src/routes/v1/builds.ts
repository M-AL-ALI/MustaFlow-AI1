...
router.get("/projects/:id/builds", async (req, res): Promise<void> => {
...
router.get("/projects/:id/builds/:buildId", async (req, res): Promise<void> => {
  const projectId = Number(req.params.id);
...
router.post("/projects/:id/builds", async (req, res): Promise<void> => {
  const projectId = Number(req.params.id);
  if (!Number.isFinite(projectId)) {
    res.status(400).json({ error: "Invalid project id." });
    return;
...
router.post("/projects/:id/builds/:buildId/cancel", async (req, res): Promise<void> => {
  const projectId = Number(req.params.id);
  const buildId = Number(req.params.buildId);
  if (!Number.isFinite(projectId) || !Number.isFinite(buildId)) {
    res.status(400).json({ error: "Invalid project id or build id." });
