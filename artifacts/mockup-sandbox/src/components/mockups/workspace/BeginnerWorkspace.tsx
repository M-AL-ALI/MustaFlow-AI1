      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
            ...
            <div className="relative z-10 w-[580px] bg-white rounded-2xl shadow-2xl shadow-black/60 overflow-hidden ring-1 ring-zinc-700/50">
              <div className="h-8 bg-zinc-100/95 flex items-center gap-1.5 px-3 border-b border-zinc-200/80">
                ...
                <div className="flex-1 mx-3 h-5 bg-white rounded-md border border-zinc-200 text-[9px] text-zinc-400 flex items-center px-2 gap-1">
                  <Lock style={{ width: 8, height: 8 }} className="text-zinc-400" /> towing-co-landing.preview
                </div>
              </div>
              <div className="h-64 bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900">
                <div className="p-6">
                  <div className="text-xs text-yellow-400 font-semibold mb-1">24/7 TOWING SERVICE</div>
                  <div className="text-white text-2xl font-bold leading-tight">When You're Stuck,<br />We've Got You.</div>
                  ...
                  <div className="mt-5 grid grid-cols-4 gap-2">
                    {["Towing", "Lockout", "Jump Start", "Tire Change"].map((s) => (
                      <div key={s} className="bg-white/5 border border-white/10 rounded-lg p-2 text-center">
              ...
              <div className="h-16 bg-white flex items-center justify-around px-6">
                {["Service Area", "About Us", "Reviews", "Contact"].map((s) => (
                  <div key={s} className="text-[10px] text-zinc-500 font-medium">{s}</div>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="w-56 bg-[#161b27] border-l border-zinc-800/60 flex flex-col p-3 gap-3 overflow-y-auto shrink-0">
            <div className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">What's next</div>
            ...
            <div className="mt-1 text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">Recent versions</div>
            {["Initial build", "Add nav bar"].map((v, i) => (
              <div key={v} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-zinc-800/50 cursor-pointer">
                ...
                {EVENTS.map((ev, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-1 shrink-0">
                      <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-[11px] ${
                        !ev.done
                      ...
                      {i < EVENTS.length - 1 && <ChevronRight className="h-3 w-3 text-zinc-700 shrink-0" />}
                    </div>
                  );
                })}
              </div>
                ...
                <div className="bg-zinc-800/70 border border-zinc-700/50 rounded-2xl rounded-tl-sm px-4 py-3 relative">
                  ...
                  <div className="mt-2.5 flex items-center gap-2">
                    <div className="flex items-center gap-1.5 text-xs text-zinc-500 flex-1">
                      ...
                      <span className="text-zinc-600">Add a testimonials section with 3 reviews…</span>
                    </div>
                    <div className="flex items-center gap-1.5">
