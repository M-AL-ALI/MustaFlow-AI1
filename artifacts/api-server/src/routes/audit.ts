...
router.get("/projects/:id/audit", requireProjectOwnership, async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!withAudit) {
      res.status(404).json({
        error: "No audit report available yet. Build or refine your app to generate one.",
      });
      return;
    }
