...
router.get(
  "/projects/:id/preview-env/status",
...
router.post(
  ...
  async (req, res) => {
    ...
    res.json({
      ...
      testingStatus: "building",
      isFullStack: true,
      message: "Test candidate created. Container is starting — poll /preview-env/status for updates.",
    });
  },
);
  ...
  async (req, res) => {
    ...
    if (project.testContainerId) {
      try {
        ...
        logger.warn({ err, projectId }, "Failed to stop old test container during rebuild — continuing");
      }
    }
    ...
    if (!fullStack) {
      await db
        ...
        .where(eq(projectsTable.id, projectId));
      res.json({ ok: true, candidateSnapshotId: candidateId, testingStatus: "ready", isFullStack: false });
      return;
    }

    await db
      .update(projectsTable)
      .set({ testingStatus: "building", testingCandidateSnapshotId: candidateId, updatedAt: new Date() })
      .where(eq(projectsTable.id, projectId));
    ...
    res.json({ ok: true, candidateSnapshotId: candidateId, testingStatus: "building", isFullStack: true });
  },
);

// ── POST /projects/:id/preview-env/stop ──────────────────────────────────────
router.post(
  "/projects/:id/preview-env/stop",
  requireProjectOwnership,
  async (req, res) => {
    const projectId = Number(req.params.id);
    ...
    if (project.testContainerId) {
      try {
        ...
        logger.warn({ err, projectId }, "Failed to stop test container during explicit stop — continuing");
      }
    }
 ...
 */
router.post(
  "/projects/:id/preview-env/approve",
  requireProjectOwnership,
  async (req, res) => {
    const projectId = Number(req.params.id);
    ...
    if (fullStack) {
      ...
      if (project.runningTestSnapshotId !== project.testingCandidateSnapshotId) {
        res.status(422).json({
          error: "The test container is running a different snapshot than the current candidate. Rebuild the test environment.",
    ...
    if (!candidateVersion) {
      res.status(422).json({ error: "Testing candidate snapshot not found.", code: "version_missing" });
      return;
    }
    if (candidateVersion.migrationStatus === "failed") {
      res.status(422).json({
        error: "Preview database migration failed for this candidate. Fix migrations before approving.",
  ...
  async (req, res) => {
    ...
    if (sessionId) {
      const [existing] = await db
        .select({ revokedAt: previewSessionsTable.revokedAt, expiresAt: previewSessionsTable.expiresAt })
    ...
    res.json({
      ...
      message: "Open launchUrl in the same browser to start the preview session. Token is single-use and expires in 10 minutes.",
    });
  },
);
...
async function startTestContainer(
  ...
  if (!containerInfo) {
    throw new Error("Container creation failed — FLY_API_TOKEN may be missing or Fly returned an error");
  }
  ...
  const healthy = await waitForContainerHealthy(
