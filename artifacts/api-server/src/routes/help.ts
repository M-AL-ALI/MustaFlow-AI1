...
router.post("/help/support/escalate", supportEscalateLimiter, async (req, res) => {
  ...
  const userEmail = await resolveUserEmail(authed.userId);
  // Default to support@mustaflow.com so tickets are always emailed even when
  // SUPPORT_EMAIL is not explicitly configured in the environment.
  const recipient = process.env.SUPPORT_EMAIL ?? "support@mustaflow.com";
