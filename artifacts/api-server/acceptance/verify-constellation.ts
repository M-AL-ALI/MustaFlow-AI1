...
async function main(): Promise<void> {
  ...
  console.log("A. Company registration (gated setup)");
  {
    ...
    check(
      "fresh user GET /org → 404 not_in_org",
      r0.status === 404 && r0.json?.code === "not_in_org",
      r0,
    );
    ...
    check(
      "registered org shape",
      reg.json?.org?.companyName === COMPANY && reg.json?.org?.status === "active",
      reg.json?.org,
    );
    ...
    check(
      "default org cap $1000",
      reg.json?.org?.effectiveSpendCapUsdCents === 100_000,
      reg.json?.org?.effectiveSpendCapUsdCents,
    );
    ...
    check(
      "second register → 409 already_in_org",
      dup.status === 409 && dup.json?.code === "already_in_org",
      dup,
    );
  }
  ...
  const [orgRow] = await db
    ...
    .where(eq(nabuflowOrgsTable.companyName, COMPANY))
    .limit(1);
  ...
  check(
    "org has company Stripe customer id",
    typeof custId === "string" && custId.startsWith("cus_"),
  );
  ...
  // Company-flagged customer (organization entity, never a personal customer)
  {
    ...
    check(
      "customer billing address captured",
      cust.address?.line1 === "1 Enterprise Way" && cust.address?.country === "US",
    );
    let linked = false;
  ...
  console.log("\nB. Volume pricing & bulk purchases");
  {
    ...
    check(
      ...
      tiers.length === 3 &&
        tiers[0]?.minCredits === 25_000 &&
        tiers[0]?.usdPerCredit === 0.009 &&
        tiers[1]?.minCredits === 100_000 &&
        tiers[1]?.usdPerCredit === 0.008 &&
        tiers[2]?.minCredits === 500_000 &&
        tiers[2]?.usdPerCredit === 0.007,
      tiers,
    );

    const below = await api(ADMIN, "POST", "/billing/nabuflow/org/purchase", {
      credits: 10_000,
      method: "card",
    });
    check(
      "below minimum → 400 below_minimum",
      below.status === 400 && below.json?.code === "below_minimum",
      below,
    );

    const terms = await api(ADMIN, "POST", "/billing/nabuflow/org/purchase", {
      credits: 25_000,
      method: "invoice",
    });
    check(
      "invoice w/o terms → 402 terms_not_enabled",
      terms.status === 402 && terms.json?.code === "terms_not_enabled",
      terms,
    );

    // No company card yet → friendly pre-charge block, purchase marked failed
    const noCard = await api(ADMIN, "POST", "/billing/nabuflow/org/purchase", {
      credits: 25_000,
      method: "card",
    });
    check(
      "card purchase w/o card → 402 no_payment_method",
      noCard.status === 402 && noCard.json?.code === "no_payment_method",
      noCard,
    );
    const failedRows = await db
      ...
      .where(
        and(
          eq(nabuflowOrgPurchasesTable.orgId, orgId),
          eq(nabuflowOrgPurchasesTable.status, "failed"),
        ),
      );
    check(
      "no-card purchase marked failed, never credited",
      failedRows.length === 1 && failedRows[0].creditedAt === null,
      failedRows.length,
    );
    const org1 = await db
      .select()
    ...
    const badPm = await stripe.paymentMethods.attach("pm_card_chargeCustomerFail", {
      customer: custId,
    });
    await stripe.customers.update(custId, {
      invoice_settings: { default_payment_method: badPm.id },
    });
    const declined = await api(ADMIN, "POST", "/billing/nabuflow/org/purchase", {
      credits: 25_000,
      method: "card",
    });
    check(
      "declined card → payment_failed",
      declined.status >= 400 && declined.json?.code === "payment_failed",
      declined,
    );
    ...
    check(
      "declined purchase marked failed, never credited",
      failedRows2.length === 2 && failedRows2.every((p) => p.creditedAt === null),
      failedRows2.length,
    );
  ...
  // SetupIntent route exists for the browser card flow; attach test card via SDK.
  {
    ...
    check(
      "org setup-intent → clientSecret",
      si.status === 200 &&
        typeof si.json?.clientSecret === "string" &&
        si.json.clientSecret.startsWith("seti_"),
      si.status,
    );
  ...
  let expectedPool = 0;
  {
    ...
    expectedPool += 25_000;
    check(
      "25k card purchase → 201 paid",
      buy1.status === 201 && buy1.json?.purchase?.status === "paid",
      buy1,
    );
    check(
      "25k priced at tier $0.009 = $225.00",
      buy1.json?.purchase?.amountUsdCents === 22_500,
      buy1.json?.purchase,
    );
    check(
      "pool funded immediately (card)",
      buy1.json?.poolCredits === expectedPool,
      buy1.json?.poolCredits,
    );
    ...
    const [p1] = await db
      ...
      .where(
        and(
          eq(nabuflowOrgPurchasesTable.orgId, orgId),
          eq(nabuflowOrgPurchasesTable.status, "paid"),
        ),
      );
    const inv1 = (await stripe.invoices.retrieve(p1.stripeInvoiceId!)) as any;
    check("Stripe invoice paid", inv1.status === "paid");
    check(
      "invoice bills the company customer",
      (typeof inv1.customer === "string" ? inv1.customer : inv1.customer?.id) === custId,
    );
    const line = inv1.lines?.data?.[0];
    check(
      "line item human-readable (credits @ rate + tier)",
      typeof line?.description === "string" &&
        line.description.includes("25,000 build credits") &&
        line.description.includes("$0.009/credit"),
      line?.description,
    );
    const cf: Array<{ name: string; value: string }> = inv1.custom_fields ?? [];
    check(
      "PO reference printed on invoice",
      cf.some((f) => f.value === "PO-E2E-001"),
      cf,
    );
    check(
      "Tax/VAT id printed on invoice",
      cf.some((f) => f.value === "EU123456789"),
      cf,
    );
    check(
      "invoice namespaced to enterprise lane",
      inv1.metadata?.surface === "nabuflow" && inv1.metadata?.purpose === "org_pool_purchase",
      inv1.metadata,
    );

    // Webhook replay is a no-op (creditedAt latch)
    await handleNabuflowOrgInvoicePaid(inv1);
    const orgAfterReplay = await db
      .select()
      .from(nabuflowOrgsTable)
      .where(eq(nabuflowOrgsTable.id, orgId))
      .limit(1);
    check(
      "invoice.paid replay does NOT double-credit",
      orgAfterReplay[0].poolCredits === expectedPool,
      orgAfterReplay[0].poolCredits,
    );

    const buy2 = await api(ADMIN, "POST", "/billing/nabuflow/org/purchase", {
      credits: 100_000,
      method: "card",
    });
    expectedPool += 100_000;
    check(
      "100k card purchase hits $0.008 tier = $800.00",
      buy2.status === 201 && buy2.json?.purchase?.amountUsdCents === 80_000,
      buy2.json?.purchase,
    );
  ...
  {
    check(
      "org invoice detected via metadata",
      await isNabuflowOrgInvoiceEvent({
        id: "in_x",
        metadata: { surface: "nabuflow", purpose: "org_pool_purchase" },
      } as any),
    );
    check(
      "personal nabuflow invoice NOT org",
      !(await isNabuflowOrgInvoiceEvent({
        id: `in_nonexistent_${ts}`,
        customer: `cus_nonexistent_${ts}`,
        metadata: { surface: "nabuflow", purpose: "subscription" },
      } as any)),
    );
    check(
  ...
  {
    const ghost = await api(ADMIN, "POST", "/billing/nabuflow/org/seats", {
      email: `ghost+${ts}@nowhere.test`,
    });
    check(
      "seat for unknown account → 404 user_not_found",
      ghost.status === 404 && ghost.json?.code === "user_not_found",
      ghost,
    );
    ...
    check(
      "seat sees org billing state",
      seatView.status === 200 && seatView.json?.org?.companyName === COMPANY,
      seatView.status,
    );
    check("seat role = member", seatView.json?.role === "member");
    check(
      "member does NOT get admin extras",
      !("purchases" in (seatView.json ?? {})) &&
        !("ledger" in (seatView.json ?? {})) &&
        !("card" in (seatView.json ?? {})),
      Object.keys(seatView.json ?? {}),
    );

    const adminView = await api(ADMIN, "GET", "/billing/nabuflow/org");
    check(
      "admin sees seats incl. member",
      Array.isArray(adminView.json?.seats) && adminView.json.seats.length === 2,
    );
    check(
      "admin sees purchases",
      Array.isArray(adminView.json?.purchases) && adminView.json.purchases.length >= 3,
    );
    check(
      "admin sees pool ledger",
      Array.isArray(adminView.json?.ledger) && adminView.json.ledger.length >= 2,
    );
    check(
      "admin sees company card last4",
      adminView.json?.card?.last4 === "4242",
      adminView.json?.card,
    );

    const state = await api(SEAT, "GET", "/billing/nabuflow/state");
    check(
      "seat state: plan = constellation",
      state.json?.plan?.id === "constellation",
      state.json?.plan?.id,
    );
    check(
      "seat state: org block present",
      state.json?.org?.companyName === COMPANY && state.json?.org?.poolCredits === expectedPool,
      state.json?.org,
    );
    ...
    check(
      "member PATCH org → 403 not_billing_admin",
      patch.status === 403 && patch.json?.code === "not_billing_admin",
      patch,
    );
    const mBuy = await api(SEAT, "POST", "/billing/nabuflow/org/purchase", {
      credits: 25_000,
      method: "card",
    });
    check("member purchase → 403", mBuy.status === 403 && mBuy.json?.code === "not_billing_admin");
    const mCap = await api(SEAT, "POST", "/billing/nabuflow/org/spend-cap", {
      spendCapUsdCents: 1,
    });
  ...
  {
    const gate = await resolveNabuflowBuildGate(SEAT, {
      engineMode: "pro",
      deepReasoning: true,
      projectedCredits: 45,
    });
    ...
    const events = await db
      ...
      .where(
        and(eq(nabuflowUsageEventsTable.userId, SEAT), eq(nabuflowUsageEventsTable.orgId, orgId)),
      );
    ...
    check(
      "events: draw valued at $0.010/credit",
      events.every((e) => e.usdValueCents === e.credits),
      events.map((e) => e.usdValueCents),
    );
    check(
      "events: zero included/overage (pool attribution)",
      events.every((e) => e.includedCredits === 0 && e.overageCredits === 0),
    );
    ...
    check(
      "usage surface shows pool draws",
      usage.status === 200 && blob.includes('"pool"'),
      usage.status,
    );
  }
  ...
  {
    const overMax = await api(ADMIN, "POST", "/billing/nabuflow/org/spend-cap", {
      spendCapUsdCents: 1_000_001,
    });
    check("org cap above $10,000 max → 400", overMax.status === 400, overMax);

    const setCap = await api(ADMIN, "POST", "/billing/nabuflow/org/spend-cap", {
      spendCapUsdCents: 100,
    });
    check(
      "org cap set to $1.00",
      setCap.status === 200 && setCap.json?.effectiveSpendCapUsdCents === 100,
    );

    // Draw to 65% of cap → 50% warning fires for admins
    await maybeChargeNabuflow(SEAT, 10, {
      type: "pipeline",
      description: "E2E threshold draw",
      engineMode: "pro",
    } as any);
    expectedPool -= 10;
    expectedMonthCredits += 10;
    ...
    for (let i = 0; i < 10 && !warn50; i++) {
      const rows = await db
        ...
        .where(
          and(
            eq(notificationsTable.recipientId, ADMIN),
            eq(notificationsTable.type, "nabuflow_org_cap_warning"),
          ),
        );
    ...
    const blocked = await resolveNabuflowBuildGate(SEAT, {
      engineMode: "pro",
      projectedCredits: 40,
    });
    check(
      "build past org cap blocked pre-start",
      blocked.allowed === false && (blocked as any).error?.code === "org_spend_cap_reached",
      blocked,
    );
    const drain = await resolveNabuflowBuildGate(SEAT, {
      engineMode: "pro",
      projectedCredits: 40,
    ...
    await maybeChargeNabuflow(SEAT, 40, {
      type: "pipeline",
      description: "E2E in-flight past cap",
      engineMode: "pro",
    } as any);
    expectedPool -= 40;
    ...
    const capGhost = await api(ADMIN, "POST", `/billing/nabuflow/org/seats/nope-${ts}/cap`, {
      seatSpendCapUsdCents: 5,
    });
    check(
      "seat cap for unknown seat → 404",
      capGhost.status === 404 && capGhost.json?.code === "seat_not_found",
    );
    const seatCap = await api(ADMIN, "POST", `/billing/nabuflow/org/seats/${SEAT}/cap`, {
      seatSpendCapUsdCents: 60,
    });
    check(
      "seat sub-cap set ($0.60)",
      seatCap.status === 200 && seatCap.json?.seat?.seatSpendCapUsdCents === 60,
    );
    const seatBlocked = await resolveNabuflowBuildGate(SEAT, {
      engineMode: "pro",
      projectedCredits: 5,
    });
    check(
      "seat sub-cap blocks pre-start",
      seatBlocked.allowed === false && (seatBlocked as any).error?.code === "org_seat_cap_reached",
      seatBlocked,
    );
    const adminGate = await resolveNabuflowBuildGate(ADMIN, {
      engineMode: "pro",
      projectedCredits: 5,
    });
    check("other seats unaffected by that sub-cap", adminGate.allowed === true, adminGate);
    const clearCap = await api(ADMIN, "POST", `/billing/nabuflow/org/seats/${SEAT}/cap`, {
      seatSpendCapUsdCents: null,
    });
    check(
      "seat sub-cap cleared",
      clearCap.status === 200 && clearCap.json?.seat?.seatSpendCapUsdCents === null,
    );
    const unblocked = await resolveNabuflowBuildGate(SEAT, {
      engineMode: "pro",
      projectedCredits: 5,
    });
  ...
  console.log("\nF. Pool exhaustion & refunds (in-flight never killed)");
  {
    ...
    check(
      "pool too small → org_pool_exhausted",
      empty.allowed === false && (empty as any).error?.code === "org_pool_exhausted",
      empty,
    );
    ...
    const inflight = await maybeChargeNabuflow(SEAT, 41, {
      type: "pipeline",
      description: "E2E in-flight on empty pool",
      engineMode: "pro",
    } as any);
    check("in-flight drain clamps display balance at 0", inflight?.newBalance === 0, inflight);
    const [negOrg] = await db
      .select()
      .from(nabuflowOrgsTable)
      .where(eq(nabuflowOrgsTable.id, orgId))
      .limit(1);
  ...
  console.log("\nG. Suspension");
  {
    ...
    check(
      "suspended org blocks new builds",
      s1.allowed === false && (s1 as any).error?.code === "org_suspended",
      s1,
    );
    const s2 = await resolveNabuflowBuildGate(SEAT, {
      engineMode: "pro",
      projectedCredits: 1,
      skipUsageChecks: true,
    });
    check(
      "suspension blocks even drain re-checks",
      s2.allowed === false && (s2 as any).error?.code === "org_suspended",
    );
    await db
      .update(nabuflowOrgsTable)
      .set({ status: "active" })
  ...
  {
    const gatePatch = await api(ADMIN, "PATCH", "/billing/nabuflow/org", {
      invoiceTermsEnabled: true,
    });
    check(
      "terms toggle is platform-gated → 403",
      gatePatch.status === 403 && gatePatch.json?.code === "terms_platform_gated",
      gatePatch,
    );
    ...
    await db
      .update(nabuflowOrgsTable)
      .set({ invoiceTermsEnabled: true })
      .where(eq(nabuflowOrgsTable.id, orgId));
    const patch = await api(ADMIN, "PATCH", "/billing/nabuflow/org", {
      termsNetDays: 45,
      poReference: "PO-STANDING-9",
    });
    check(
      "admin updates net days + standing PO",
      patch.status === 200 &&
        patch.json?.org?.termsNetDays === 45 &&
        patch.json?.org?.poReference === "PO-STANDING-9",
      patch.json?.org,
    );

    const buy = await api(ADMIN, "POST", "/billing/nabuflow/org/purchase", {
      credits: 25_000,
      method: "invoice",
    });
    check(
      "terms purchase → 201 pending",
      buy.status === 201 && buy.json?.purchase?.status === "pending",
      buy,
    );
    check(
      "pool NOT funded until invoice.paid",
      buy.json?.poolCredits === expectedPool,
      buy.json?.poolCredits,
    );
    ...
    const [pRow] = await db
      ...
      .where(
        and(
          eq(nabuflowOrgPurchasesTable.orgId, orgId),
          eq(nabuflowOrgPurchasesTable.status, "pending"),
        ),
      );
    ...
    const invDueDays = inv.due_date
      ? Math.round((inv.due_date * 1000 - Date.now()) / 86_400_000)
      : -1;
    check(
      "Stripe invoice open + send_invoice + net-45",
      inv.status === "open" &&
    ...
    check(
      "standing PO printed on terms invoice",
      (inv.custom_fields ?? []).some((f: any) => f.value === "PO-STANDING-9"),
      inv.custom_fields,
    );
    ...
    let [orgNow] = await db
      ...
      .limit(1);
    check(
      "webhook funds pool on invoice.paid",
      orgNow.poolCredits === expectedPool,
      orgNow.poolCredits,
    );
    await handleNabuflowOrgInvoicePaid(inv);
    [orgNow] = await db
    ...
    const [paidRow] = await db
      .select()
      .from(nabuflowOrgPurchasesTable)
      .where(eq(nabuflowOrgPurchasesTable.id, pRow.id))
      .limit(1);
    ...
    const [failedRow] = await db
      .select()
      .from(nabuflowOrgPurchasesTable)
      .where(eq(nabuflowOrgPurchasesTable.id, pRow2.id))
      .limit(1);
    check(
      "payment_failed marks purchase failed, pool untouched",
      failedRow.status === "failed" && failedRow.creditedAt === null,
    );
    const failNote = await db
      ...
      .where(
        and(
          eq(notificationsTable.recipientId, ADMIN),
          eq(notificationsTable.type, "nabuflow_org_purchase_failed"),
        ),
      );
    ...
    const [recovered] = await db
      ...
      .limit(1);
    check(
      "late payment recovers the purchase",
      recovered.status === "paid" && recovered.creditedAt !== null,
    );
    const fundedNote = await db
      ...
      .where(
        and(
          eq(notificationsTable.recipientId, ADMIN),
          eq(notificationsTable.type, "nabuflow_org_pool_funded"),
        ),
      );
  ...
  {
    const [orgFinal] = await db
      .select()
      .from(nabuflowOrgsTable)
      .where(eq(nabuflowOrgsTable.id, orgId))
      .limit(1);
    check("final pool matches running expectation", orgFinal.poolCredits === expectedPool, {
      actual: orgFinal.poolCredits,
      expectedPool,
    });
    ...
    const ledger = await db
      ...
      .where(eq(nabuflowOrgLedgerTable.orgId, orgId));
    ...
    check(
      ...
      ledger.some((l) => l.entryType === "purchase") &&
        ledger.some((l) => l.entryType === "draw") &&
        ledger.some((l) => l.entryType === "reversal"),
    );
    ...
    check("org month credits reconcile", month?.creditsDrawn === expectedMonthCredits, {
      actual: month?.creditsDrawn,
      expectedMonthCredits,
    });
    check("org month cents reconcile", month?.drawnUsdCents === expectedMonthCents, {
      actual: month?.drawnUsdCents,
      expectedMonthCents,
    });
    ...
    check(
      "seat month mirrors seat draws",
      seatMonth?.creditsDrawn === expectedMonthCredits &&
        seatMonth?.drawnUsdCents === expectedMonthCents,
      seatMonth,
    );
    ...
    const liveEvents = await db
      ...
      .from(nabuflowUsageEventsTable)
      .where(
        and(eq(nabuflowUsageEventsTable.orgId, orgId), isNull(nabuflowUsageEventsTable.reversedAt)),
      );
    const eventSum = liveEvents.reduce((s, e) => s + e.credits, 0);
    check("non-reversed usage events = month counters", eventSum === expectedMonthCredits, {
      eventSum,
    });
    ...
    const purchases = await db
      .select()
      .from(nabuflowOrgPurchasesTable)
      .where(eq(nabuflowOrgPurchasesTable.orgId, orgId));
    const creditedSum = purchases
      .filter((p) => p.creditedAt !== null)
      .reduce((s, p) => s + p.credits, 0);
    check(
      "credited purchases − draws + reversals = pool",
      creditedSum - expectedMonthCredits === orgFinal.poolCredits,
      { creditedSum },
    );
    check(
      ...
      purchases.every((p) =>
        p.credits === 25_000
          ? p.amountUsdCents === 22_500
          : p.credits === 100_000
            ? p.amountUsdCents === 80_000
            : false,
      ),
  ...
  console.log("\nJ. Ora independence");
  {
    ...
    check(
      ...
      JSON.stringify(oraCreditsBefore.json?.balance ?? oraCreditsBefore.json) ===
        JSON.stringify(oraAfter.json?.balance ?? oraAfter.json),
  ...
  console.log("\nK. Seat lifecycle");
  {
    ...
    check(
      "last billing admin cannot be removed",
      lastAdmin.status === 400 && lastAdmin.json?.code === "last_billing_admin",
      lastAdmin,
    );
    ...
    const personal = await resolveNabuflowBuildGate(SEAT, {
      engineMode: "pro",
      projectedCredits: 1,
    });
    check(
      "removed seat falls back to personal lane (no_plan)",
      personal.allowed === false && (personal as any).error?.code === "no_plan",
      personal,
    );
  }
