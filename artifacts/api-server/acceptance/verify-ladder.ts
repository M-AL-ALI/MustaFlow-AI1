...
async function main(): Promise<void> {
  ...
  {
    const noPlan = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "eco",
      projectedCredits: 5,
    });
    check(
      "no plan → no_plan block",
      !noPlan.allowed && gateErr(noPlan)?.code === "no_plan",
      noPlan,
    );
    ...
    const noCard = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "eco",
      projectedCredits: 5,
    });
    check(
      "no card on file → no_payment_method",
      !noCard.allowed && gateErr(noCard)?.code === "no_payment_method",
      noCard,
    );
    ...
    const expired = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "eco",
      projectedCredits: 5,
    });
    check(
      "expired card → card_expired",
      !expired.allowed && gateErr(expired)?.code === "card_expired",
      expired,
    );
    ...
    await db
      ...
      .where(eq(nabuflowSubscriptionsTable.userId, ORBIT));
    const paused = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "eco",
      projectedCredits: 5,
    });
    check(
      "dunning paused → billing_paused",
      !paused.allowed && gateErr(paused)?.code === "billing_paused",
      paused,
    );
    await db
      .update(nabuflowSubscriptionsTable)
  ...
  const orbitResetsAt = orbitSub0!.currentCycleEnd!.toISOString();
  {
    ...
    check(
      "state: fresh cycle counter 3 remaining",
      s0.json?.cycle?.remainingProBuilds === 3 && s0.json?.cycle?.proBuildsUsed === 0,
      s0.json?.cycle,
    );
    check(
      "state: reset date visible",
      s0.json?.cycle?.resetsAt === orbitResetsAt,
      s0.json?.cycle?.resetsAt,
    );

    // Orbit cannot enable Deep at all
    const deep = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "eco",
      deepReasoning: true,
      projectedCredits: 15,
    });
    check(
      "Orbit Deep → mode_not_available",
      !deep.allowed && gateErr(deep)?.code === "mode_not_available",
      deep,
    );
    check(
      "Deep block hints an upgrade",
      typeof gateErr(deep)?.upgradeTarget === "string" && !!gateErr(deep)?.upgradeTarget,
    );
    const proDeep = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "pro",
      deepReasoning: true,
      projectedCredits: 45,
    });
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      await maybeChargeNabuflow(ORBIT, 10, {
        type: "build",
        description: `E2E orbit pro #${i}`,
        engineMode: "pro",
      });
      const s = await api(ORBIT, "GET", "/billing/nabuflow/state");
      check(
        `counter after #${i}: ${3 - i} remaining`,
        s.json?.cycle?.remainingProBuilds === 3 - i && s.json?.cycle?.proBuildsUsed === i,
        s.json?.cycle,
      );
    }

    // The 4th Pro build is blocked PRE-START with the calm upgrade prompt
    const fourth = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "pro",
      projectedCredits: 10,
    });
    const e4 = gateErr(fourth);
    check(
      "4th Pro build blocked pre-start",
      !fourth.allowed && e4?.code === "mode_limit_reached",
      fourth,
    );
    ...
    check(
      "calm copy mentions limit + reset",
      /used all 3 Pro builds/.test(e4?.message ?? "") && /reset/i.test(e4?.message ?? ""),
      e4?.message,
    );
    ...
    const power = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "power",
      projectedCredits: 10,
    });
    check("Power still allowed at Pro limit", power.allowed === true);
    const drain = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "pro",
      projectedCredits: 10,
    ...
    check(
      "refund releases a Pro slot (counter 2/3)",
      sAfterRefund.json?.cycle?.proBuildsUsed === 2 &&
        sAfterRefund.json?.cycle?.remainingProBuilds === 1,
      sAfterRefund.json?.cycle,
    );
    const again = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "pro",
      projectedCredits: 10,
    });
    ...
    await maybeChargeNabuflow(ORBIT, 10, {
      type: "build",
      description: "E2E orbit pro #4 (re-book)",
      engineMode: "pro",
    });
    const reblocked = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "pro",
      projectedCredits: 10,
    });
    check(
      "back at 3/3 → blocked again",
      !reblocked.allowed && gateErr(reblocked)?.code === "mode_limit_reached",
    );
    ...
    check(
      "architect charge does not tick Pro counter",
      sArch.json?.cycle?.proBuildsUsed === 3,
      sArch.json?.cycle,
    );
  }
  ...
  {
    const combo = await resolveNabuflowBuildGate(COMET, {
      engineMode: "pro",
      deepReasoning: true,
      projectedCredits: 45,
    });
    ...
    check(
      "Comet Pro+Deep → combo_not_available",
      !combo.allowed && ec?.code === "combo_not_available",
      combo,
    );
    check(
      "combo block prompts Nova",
      ec?.upgradeTarget === "nova" && /Nova/.test(ec?.message ?? ""),
      ec,
    );

    for (let i = 1; i <= 10; i++) {
      const g = await resolveNabuflowBuildGate(COMET, {
        engineMode: "power",
        deepReasoning: true,
        projectedCredits: 15,
      });
      check(`Deep build #${i} allowed`, g.allowed === true, g);
      await maybeChargeNabuflow(COMET, 15, {
        type: "build",
        description: `E2E comet deep #${i}`,
        engineMode: "power",
        deepReasoning: true,
      });
    }
    ...
    check(
      "counter shows 10/10 Deep used",
      s10.json?.cycle?.deepBuildsUsed === 10 && s10.json?.cycle?.remainingDeepBuilds === 0,
      s10.json?.cycle,
    );

    const eleventh = await resolveNabuflowBuildGate(COMET, {
      engineMode: "power",
      deepReasoning: true,
      projectedCredits: 15,
    });
    ...
    check(
      "11th Deep build blocked pre-start",
      !eleventh.allowed && e11?.code === "mode_limit_reached",
      eleventh,
    );
    check(
      "block shows 0 Deep remaining + reset",
      e11?.remainingDeepBuilds === 0 && e11?.resetsAt === cometSeed.end.toISOString(),
      e11,
    );
    check(
      "calm copy: used all 10 Deep builds",
      /used all 10 Deep builds/.test(e11?.message ?? ""),
      e11?.message,
    );
    check("upgrade prompt targets Nova", e11?.upgradeTarget === "nova", e11?.upgradeTarget);

    const proStill = await resolveNabuflowBuildGate(COMET, {
      engineMode: "pro",
      projectedCredits: 10,
    });
    check("Comet Pro is unlimited (allowed at Deep limit)", proStill.allowed === true, proStill);
    const plain = await resolveNabuflowBuildGate(COMET, {
      engineMode: "power",
      projectedCredits: 10,
    });
  ...
  console.log("\nD. Nova exclusivity");
  {
    ...
    const combo = await resolveNabuflowBuildGate(NOVA, {
      engineMode: "pro",
      deepReasoning: true,
      projectedCredits: 45,
    });
    check("Nova Pro+Deep allowed", combo.allowed === true, combo);
    await maybeChargeNabuflow(NOVA, 45, {
      type: "build",
      description: "E2E nova pro+deep",
      engineMode: "pro",
      deepReasoning: true,
    });
    const s = await api(NOVA, "GET", "/billing/nabuflow/state");
    check(
      "Nova counters unlimited (null remaining)",
      s.json?.cycle?.remainingProBuilds === null && s.json?.cycle?.remainingDeepBuilds === null,
      s.json?.cycle,
    );
    check(
      "Nova counters still tracked",
      s.json?.cycle?.proBuildsUsed === 1 && s.json?.cycle?.deepBuildsUsed === 1,
      s.json?.cycle,
    );
  }
  ...
  console.log("\nE. Cycle rollover (counters reset, rollover credits honored)");
  {
    ...
    const [oldCycle] = await db
      ...
      .where(
        and(
          eq(nabuflowBillingCyclesTable.userId, ORBIT),
          eq(nabuflowBillingCyclesTable.cycleStart, subBefore!.currentCycleStart!),
        ),
      );
    ...
    const afterRoll = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "pro",
      projectedCredits: 10,
    });
    ...
    check(
      "fresh cycle: counters reset to 0/3",
      sRoll.json?.cycle?.proBuildsUsed === 0 && sRoll.json?.cycle?.remainingProBuilds === 3,
      sRoll.json?.cycle,
    );
    check(
      "Orbit rollover policy: no credits carry",
      sRoll.json?.cycle?.includedCredits === NABUFLOW_PLANS.orbit.includedMonthlyCredits,
      sRoll.json?.cycle?.includedCredits,
    );
    ...
    const [oldRow] = await db
      ...
      .where(eq(nabuflowBillingCyclesTable.id, oldCycle.id));
    check(
      "old cycle history intact (3 Pro used)",
      oldRow.proBuildsUsed === 3 && oldRow.usedIncludedCredits === oldCycle.usedIncludedCredits,
    );
    ...
    const [cometCycle] = await db
      ...
      .where(
        and(
          eq(nabuflowBillingCyclesTable.userId, COMET),
          eq(nabuflowBillingCyclesTable.cycleStart, cometBefore!.currentCycleStart!),
        ),
      );
    ...
    const deepAgain = await resolveNabuflowBuildGate(COMET, {
      engineMode: "power",
      deepReasoning: true,
      projectedCredits: 15,
    });
    ...
    check(
      "Comet Deep counter reset (0/10)",
      sC.json?.cycle?.deepBuildsUsed === 0 && sC.json?.cycle?.remainingDeepBuilds === 10,
      sC.json?.cycle,
    );
    check(
      "Comet unused credits rolled one cycle (capped)",
      sC.json?.cycle?.includedCredits ===
        NABUFLOW_PLANS.comet.includedMonthlyCredits + expectedRollover,
      { actual: sC.json?.cycle?.includedCredits, expectedRollover },
    );
    const cometAfter = await getNabuflowSubscription(COMET);
    check(
      "rollover persisted on subscription",
      cometAfter!.rolloverCredits === expectedRollover,
      cometAfter!.rolloverCredits,
    );
  }
  ...
  console.log("\nF. Overage invoice items, spend cap, usage reconciliation");
  {
    ...
    await db
      ...
      .where(
        and(
          eq(nabuflowBillingCyclesTable.userId, ORBIT),
          eq(nabuflowBillingCyclesTable.cycleStart, orbitSub!.currentCycleStart!),
        ),
      );

    const charge = await maybeChargeNabuflow(ORBIT, 100, {
      type: "build",
      description: "E2E overage build",
      engineMode: "power",
    });
    check(
      "overage charge accepted (never blocks in-flight)",
      charge !== null && charge!.newBalance === 0,
      charge,
    );
    ...
    const [ovEvent] = await db
      ...
      .where(
        and(
          eq(nabuflowUsageEventsTable.userId, ORBIT),
          eq(nabuflowUsageEventsTable.attribution, "overage"),
        ),
      );
    check(
      "overage event: 100cr @ $0.015 = 150¢",
      ovEvent?.overageCredits === 100 && ovEvent?.overageUsdCents === 150,
      ovEvent,
    );
    ...
    if (itemId) {
      ...
      check(
        "invoice item bills the right customer",
        (typeof item.customer === "string" ? item.customer : item.customer?.id) === cust.id,
      );
      check("invoice item amount = ledger cents", item.amount === 150, item.amount);
      check(
        "invoice item line is human-readable",
        typeof item.description === "string" &&
          /pay-as-you-go|overage/i.test(item.description) &&
          /100 credits/.test(item.description),
        item.description,
      );
    }

    // Spend cap: projected overage past the cap blocks pre-start only.
    const capBlock = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "eco",
      projectedCredits: 2100,
    });
    check(
      "projected overage past cap → spend_cap_reached",
      !capBlock.allowed && gateErr(capBlock)?.code === "spend_cap_reached",
      capBlock,
    );
    const capDrain = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "eco",
      projectedCredits: 2100,
      skipUsageChecks: true,
    });
    check("cap never kills in-flight builds", capDrain.allowed === true);
    const smallOk = await resolveNabuflowBuildGate(ORBIT, {
      engineMode: "eco",
    ...
    const liveEvents = await db
      ...
      .where(
        and(
          eq(nabuflowUsageEventsTable.userId, ORBIT),
          eq(nabuflowUsageEventsTable.cycleId, cycleNow.id),
    ...
    const cycleUsed =
      cycleNow.usedIncludedCredits -
      NABUFLOW_PLANS.orbit.includedMonthlyCredits +
      cycleNow.overageCredits; // seeded exhaustion offset
    check("event credits reconcile with cycle counters", evCredits === cycleUsed, {
      evCredits,
      cycleUsed,
    });
    check("event overage cents reconcile with cycle", evOverageCents === cycleNow.overageUsdCents, {
      evOverageCents,
      cycle: cycleNow.overageUsdCents,
    });
    const proEvents = liveEvents.filter(
      (e) =>
        e.engineMode === "pro" &&
        (e.source === "pipeline" || e.source === "background" || e.source === "queue"),
    );
    check("pro-build events == proBuildsUsed", proEvents.length === cycleNow.proBuildsUsed, {
      n: proEvents.length,
      c: cycleNow.proBuildsUsed,
    });

    const usage = await api(ORBIT, "GET", "/billing/nabuflow/usage");
    const blob = JSON.stringify(usage.json ?? {});
    check(
      "usage dashboard shows the overage event",
      usage.status === 200 && blob.includes("overage"),
      usage.status,
    );
  }
  ...
  console.log("\nG. Ora independence");
  {
    ...
    check(
      "Ora credits API unaffected for ladder users",
      ora.status === 200 && ora.json !== null,
      ora.status,
    );
  }
