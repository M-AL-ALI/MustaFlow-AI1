...
- Application code may read only process.env.PORT for the HTTP listener. Do not read any other environment variable, including NODE_ENV, import.meta.env.MODE, DATABASE_URL, STRIPE_*, credentials, or API keys. Use explicit non-secret application defaults instead of environment-based development/production branches. Do not disguise forbidden reads with bracket access or aliases.
...
- When the app declares the database capability, bind the HTTP listener before awaiting database capability or schema initialization so GET /healthz stays available while the capability is being provisioned. Start one retryable app-owned initialization promise after binding, initialize the schema idempotently through createNabuFlowDatabase (for example, CREATE TABLE IF NOT EXISTS), and make database routes await that promise before querying. A capability or schema failure returns a sanitized typed 503 from database routes; it must not terminate the process or make healthz fail. Do not invoke migration CLIs or read raw database credentials.
- The source package's start script remains "node dist/src/index.js". The sealed runtime manifest is fixed at port 8080, health path /healthz, build argv ["npm","run","build"], and sealed-output start argv ["node","src/index.js"].`;
