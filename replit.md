...
- **DeepSeek (Ora smart-routing)**: `DEEPSEEK_API_KEY` — adds DeepSeek as a fourth Ora provider (alongside OpenAI/Anthropic/Gemini). DeepSeek is **not** carried by the AI-integrations proxy, so it uses a direct OpenAI-compatible client (`baseURL: https://api.deepseek.com`, `max_tokens` not `max_completion_tokens`). Ora's conversational branch classifies each message and picks the best provider via `lib/public-ai/model-router.ts` (`selectOraModelRoute`), with availability-aware ordered fallback that always ends on OpenAI. When the key is unset, DeepSeek silently no-ops (filtered out of the candidate chain; `resolveStageProvider` falls back to OpenAI). Models: `deepseek-chat` (fast/eco tiers), `deepseek-reasoner` (power/pro/deep tiers). DeepSeek has no vision model — vision turns fall back to OpenAI.
