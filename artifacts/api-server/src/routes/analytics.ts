...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const avgDurationMs =
      total > 0
        ? Math.round(rows.reduce((sum, r) => sum + r.durationMs, 0) / total)
        : null;
    const avgCorrectionPasses =
      total > 0
        ? rows.reduce((sum, r) => sum + r.correctionPasses, 0) / total
        : null;
    ...
    for (const r of rows) {
      if (r.primaryErrorCategory) {
        errorCounts.set(
          r.primaryErrorCategory,
          (errorCounts.get(r.primaryErrorCategory) ?? 0) + 1,
        );
      }
    }
