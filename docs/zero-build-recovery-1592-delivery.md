...
- API focused regressions: 3 files, 32 tests, all PASS.
- Web focused regression: 1 file, 2 tests, all PASS.
...
- API full-suite failure-set parity:
  ...
  - branch: 39 failing / 160 passing / 3 skipped files; 41 failing / 2,421 passing / 5 skipped tests.
  - delta: one new passing file and nine new passing tests; no new failing file or test.
- Web full-suite failure-set parity:
  - base: 1 failing / 104 passing files; 1 failing / 1,060 passing tests.
  - branch: 1 failing / 104 passing files; 1 failing / 1,061 passing tests.
  - delta: one new passing test; the sole pre-existing captured-frame hash failure is identical.
...
Pending the normal Replit merge, gate, and publish ritual. After the ship, Project 52 (`IRQ TEL`) must successfully build the founder's Iraqi-flag website request through the real product route.
