...
The preview pane is an <iframe> connected to the dev server through the NabuFlow reverse proxy. The proxy is byte-transparent: it forwards HTTP requests, WebSocket upgrades, and SSE streams unchanged. Whatever the server returns is exactly what the iframe shows.
...
- Never fail a task solely because npm install cannot complete — the server may already be running.`
  ...
  return [
    isDeveloperMode
      ...
      : `You are NabuFlow's agentic app builder. Your job is to ${input.mode === "build" ? "create" : "refine"} a working ${stack} application that satisfies the user's request.`,
    "",
    platformNote,
    ...
    "",
    "## Test-fix loop",
    "- When your task is to fix failing tests, always start with `run_tests` to get the current pass/fail status before touching any code.",
    "- The `run_tests` observation includes a structured JSON result with `failedTests: [{ name, message }]`. Use the failing test names and error messages to locate the relevant source files with `search` and `read_file`.",
    "- Fix one failure category at a time: read the failing test file to understand the contract → read the implementation being tested → apply a targeted fix with `apply_patch` (preferred for surgical edits) or `write_file`.",
    "- After each fix batch, call `run_tests` again to verify progress. Repeat the loop (run_tests → read failures → patch code → run_tests) until `failed === 0` or the step cap is hit.",
    "- Never modify passing tests to accommodate broken code. Fix the implementation, not the test expectations, unless the test expectation is demonstrably wrong.",
    ...
    "- If eslint is not installed (`command not found`), report this clearly in `finalize` rather than trying to install it.",
...
${inventory || "(empty workspace)"}`;
        ...
        md.push("");
        md.push(`_Generated by NabuFlow threat_model tool._`);
        md.push("");
        if (parsed.summary) {
          md.push(`## Summary`);
