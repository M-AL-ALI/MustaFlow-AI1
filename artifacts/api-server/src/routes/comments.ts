...
router.get("/projects/:id/comments", requireProjectOwnership, async (req, res): Promise<void> => {
...
router.post("/projects/:id/comments", requireProjectOwnership, async (req, res): Promise<void> => {
  const projectId = parseInt(pstr(req.params.id), 10);
  ...
  if (parsed.data.parentId != null) {
    ...
    if (!parent || parent.projectId !== projectId) {
      res.status(400).json({ error: "Parent comment not found in this project" });
      return;
    }
  }
