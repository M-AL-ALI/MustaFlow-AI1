/**
 * File validation for Ora Phase 2.
 *
 * Two-layer check: magic bytes (manual, no external package) + DOCX structure scan.
 * Returns a ValidationResult with either an allowed file type or a safe error message.
 */
...
/**
 * Verify DOCX structure by scanning the ZIP local file headers for
 * the required DOCX entry names without loading a full ZIP library.
 * Both "word/" entries and "[Content_Types].xml" must be present.
 */
...
export function validateFile(
  ...
  if (BLOCKED_EXTENSIONS.has(ext)) {
    return {
      ok: false,
      statusCode: 415,
      error: `File type "${ext}" is not supported. Please upload a PDF, DOCX, or TXT file.`,
    };
  }
  ...
  return {
    ok: false,
    statusCode: 415,
    error: `Unsupported file type "${ext || "(none)"}". Ora accepts PDF, DOCX, and TXT files only.`,
  };
}
