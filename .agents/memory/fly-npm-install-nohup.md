...
**Why (429 double-exec):** `flyFetch` retries on HTTP 429. If the first exec request reached the container and started a `nohup npm install` before being rate-limited, the retry starts a *second* concurrent npm install. Two concurrent npm installs always OOM on these containers.
...
- For one-off fix scripts: use synchronous npm install only for individual packages as a *probe* to check if it works fast (unlikely) — expect OOM, then fall back to nohup approach.
