  ...
  async (req, res): Promise<void> => {
    ...
    if (!cfEnabled()) {
      res.json({
        ...
        records: [],
        message:
          "DNS record management requires Cloudflare integration (CF_ZONE_ID + CF_API_TOKEN).",
      });
      return;
    }

    const records = await listDnsRecords(domain.hostname);
    res.json({ enabled: true, hostname: domain.hostname, records });
  },
);
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!cfEnabled()) {
      res.status(503).json({
        error: "DNS record management requires Cloudflare integration (CF_ZONE_ID + CF_API_TOKEN).",
      });
      return;
    }
