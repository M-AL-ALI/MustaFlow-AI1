...
//
// Used to back existing fixed-window rate-limit counters with a shared store so
// per-IP limits survive process restarts and span multiple instances. This is
// additive for broad IP limits: they retain their historical memory fallback.
// Cost-bearing brainstorm admission uses Postgres and is not implemented here.
//
...
import { redisRateLimitBackendAvailable } from "./metrics";

const REDIS_URL = process.env.UPSTASH_REDIS_REST_URL;
const REDIS_TOKEN = process.env.UPSTASH_REDIS_REST_TOKEN;

redisRateLimitBackendAvailable.set(0);
...
async function redisCommand(args: Array<string | number>): Promise<unknown> {
  ...
  try {
    ...
    redisRateLimitBackendAvailable.set(1);
    return json.result;
  } catch (error) {
    redisRateLimitBackendAvailable.set(0);
    throw error;
